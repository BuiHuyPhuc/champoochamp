...
class VariantChoice extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      sizes: groupBy(product.productVariant.filter(item => item.colorId === colors[0].colorId), p => p.sizeId).map(item => item.size),
    };    
  ...
  getSelectedColor = selectedColor => {
    ...
    this.setState({
      selectedColor,
      sizes: groupBy(product.productVariant.filter(item => item.colorId === selectedColor.colorId), p => p.sizeId).map(item => item.size),
    });
    ...
    return (
      <Wrapper>
        <ChoiceBox>
          ...
          <ColorRow colors={colors} size={30} selectedColor={selectedColor} getSelectedColor={this.getSelectedColor} />
        </ChoiceBox>
        <ChoiceBox>
